...
export class UnitGrid {
  private grid: Map<UnitType, Set<Unit>>[][];
  ...
  removeUnitByTile(unit: Unit, tile: TileRef) {
    const [gridX, gridY] = this.getGridCoords(this.gm.x(tile), this.gm.y(tile));
  ...
  nearbyUnits(
    ...
    const nearby: Array<{ unit: Unit; distSquared: number }> = [];
    const gm = this.gm;
    const x = gm.x(tile);
    const y = gm.y(tile);
